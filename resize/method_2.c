...
//
static int resize_method_2(struct resizeinfo *ri)
{
    // same cluster size?
	...
	if (remap_directory_entries(ri)) return 1;

	{
		...
		cluster_t rootcno = ri->rootdir->head_cluster;
...
{
	if (cluno==EXFAT_CLUSTER_END || cluno==EXFAT_CLUSTER_FREE) return cluno;

	off_t old = c2o(&ri->osb,cluno);
...
static bool isNeedMoved(struct resizeinfo *ri, struct exfat_entry *ent)
{
	...
	do {
		if (c2o(&ri->osb,cluno) <= headofs) return true;
...
static bool remap_dataclusters(struct resizeinfo *ri, struct exfat_entry *ent)
{
	...
	if (info1->type != EXFAT_ENTRY_FILE) return false;
	...
	if (isNeedMoved(ri,ent)) {
		...
		do {
			...
			if (cluno==0) return false;
			if (prev==0) info2->start_cluster = cpu_to_le32(cluno);
			else fatset_s(ri,prev,cluno);
			if (ri->writeenable) {
				off_t fo = c2o(&ri->osb,orgc);
				off_t to = c2o(&ri->sb,cluno);
				if (fo!=to) {
					if (exfat_pread( ri->dev,ri->cludata,clusize,fo) < 0) return false;
					if (exfat_pwrite(ri->dev,ri->cludata,clusize,to) < 0) return false;
				}
			}
			prev = cluno;
