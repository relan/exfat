...
static void bmpset_s(struct resizeinfo *ri, cluster_t clusno, int used)
{
	...
	unsigned char *bmp = ri->bmpdata;
	unsigned char f = 1 << (clusno % 8);
	if (used & 1) bmp[clusno/8] |= f;
	else bmp[clusno/8] &= (0xFF^f);
}

static bool bmpget(struct resizeinfo *ri, cluster_t clusno)
{
	if (clusno < 2) return false;
	clusno -= 2;
...
{
	for (off_t i = start; i <= end; ++i) bmpset_s(ri,i,f);
}
...
{
	if (ri->is_contiguous) {
		if (ri->linkcount!=0) ri->linkcount--;
		return (ri->linkcount==0) ? EXFAT_CLUSTER_END : (cluno+1);
	}
	else {
		cluster_t next = fatget(ri,cluno);	
		if (next == EXFAT_CLUSTER_FREE || next == EXFAT_CLUSTER_BAD) next = EXFAT_CLUSTER_END;
		return next;
	}
}
...
{
	le16_t tmpbuf[16];
	for (int i=0; i <= 14; ++i) { tmpbuf[i] = name[i]; tmpbuf[i+1] = cpu_to_le16(0); }; 
...
static int commit_superblock(struct resizeinfo *ri)
{
	...
	for (i = 0; i < 11; i++)
	{
		...
		if (i==0) vbr_checksum = exfat_vbr_start_checksum(secdata, secsize);
		else vbr_checksum = exfat_vbr_add_checksum(secdata, secsize, vbr_checksum);
	}
...
static struct resizeinfo *init_resizeinfo(struct exfat_dev *dev, off_t volsize)
{
	...
	ri->dev = dev;
	ri->writeenable = true; // MEMO: write enable
			...
			if (ents->type == EXFAT_ENTRY_FILE_INFO) {
				...
				if (info1->type == EXFAT_ENTRY_FILE) {
					...
					if (le16_to_cpu(info1->attrib) & EXFAT_ATTRIB_DIR) {
						if (create_dirinfo(ri,(info2->flags & EXFAT_FLAG_CONTIGUOUS)!=0,headcluno)==NULL) return 1;
						++ri->dirs;
					}
					else ++ri->files;
				}
...
static int expand_systemArea(struct resizeinfo *ri)
{
	...
	off_t bmpcno   = EXFAT_FIRST_DATA_CLUSTER;
	p = malloc(bmpsizeR);
	if (p==NULL) {
		exfat_error("failed to allocate memory");
		return 1;
